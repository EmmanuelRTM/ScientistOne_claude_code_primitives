...
| External knowledge restriction | "A paper you did not retrieve and read does not exist"; unknown citation keys are blocked at write time; offline mode restricts sources to `workspace/seeds/` | `citation_guard.py` PreToolUse hook (blocks the Write, Edit **or Bash** call — a shell redirect into a paper file is checked the same way) |
